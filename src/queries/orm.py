...
class SyncORM:
    ...
    def select_workers_with_joined_relationship(): 
        # но тут мы берем из бд больше чем нужно, есть дублирование т.к. one to many связь
        # joinedload только для подгрузки  one to one || many to one  связей
        # selectinload - many to many // one to many
        with session_factory() as session:
            query = (
                select(WorkersOrm)
                .options(joinedload(WorkersOrm.resumes)) # m2o o2o
            )
            
            res = session.execute(query)
            result = res.unique().scalars().all() # unique - убирает это дублирование после принятия ответа от бд
    ...
    def select_workers_with_selectin_relationship():
        with session_factory() as session:
            query = (
                select(WorkersOrm)
                .options(selectinload(WorkersOrm.resumes)) # o2m m2m
            )
